          ...
          {caseStudies.map((cs, i) => (
            <Reveal key={cs.slug} delay={i * 80}>
              <Link
                to="/work/$slug"
                params={{ slug: cs.slug }}
                className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card p-8 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:border-accent"
              >
                ...
                <div className="mt-auto pt-8">
                  ...
                  <p className="mt-6 text-sm font-medium text-accent transition-transform duration-300 group-hover:translate-x-2">
                    Read case study →
                  </p>
                </div>
              </Link>
